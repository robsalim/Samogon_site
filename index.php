...
$result_graph = mysqli_query($db, "SELECT datetime, t1, t2, pd, pr, p1 FROM Samog1 
                                 WHERE datetime >= '$last24hours' AND pr = '$pr'
...
while ($row = mysqli_fetch_assoc($result_graph)) {
    ...
    $corrected_time = date('Y-m-d H:i:s', strtotime($row['datetime']) + 5 * 3600);
    $row['datetime'] = $corrected_time;
    
    if (is_numeric($row['t1']) && is_numeric($row['t2']) && is_numeric($row['pd']) && is_numeric($row['p1'])) {
        $graph_data[] = $row;
    }
}

// Запрос последней записи для таблицы
$result_last = mysqli_query($db, "SELECT * FROM Samog2 WHERE pr='$pr' ORDER BY datetime DESC LIMIT 1");
