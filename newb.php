...
$p1 = intval($_REQUEST['pw2'] ?? 0);//*1.94*1.94 *15;
$p2 = intval($_REQUEST['pw3'] ?? 0);//*1.94*1.94 *15;
$cm = intval($_REQUEST['cm'] ?? 0);
$imode = intval($_REQUEST['imode'] ?? 0);
$imenu = intval($_REQUEST['imenu'] ?? 0);
$phase = intval($_REQUEST['phase'] ?? 0);
$phaseTime = intval($_REQUEST['phaseTime'] ?? 0);
...
$pressure = floatval($_REQUEST['pressure'] ?? 0);
...
if ($pr == 1) {
    try {
        ...
        $stmt1->execute([$t1, $t2, $pr, $pd, $date,$p1,$p2, $pressure]);
        ...
        $stmt2->execute([
            ...
            $cm, $imode, $imenu, $phase, $phaseTime, $spnt, 
            $k_int, $k_der, $kp, $ki, $kd, $date, $pressure]);
